...
from typing import Any, List, Coroutine, Callable, TypeVar, Tuple, Optional, Dict
...
class AsyncCache:
    ...
    async def get(self, key: str, factory: Callable[[], Coroutine[Any, Any, T]], ttl: Optional[float] = None) -> T:
        ...
        try:
            ...
            logger.error(f"Cache factory error for key '{key}': {str(e)}", exc_info=True)
            raise
        ...
        async with self.lock:
            ...
            while len(self.cache) > self.max_size:
                _popped_key, _ = self.cache.popitem(last=False)
                self.evictions += 1
    ...
    async def _cleanup_expired(self):
        if self.lock.locked():
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug("AsyncCache: Cleanup skipped, lock is held.")
            return
        ...
        if cleaned_count > 0 and logger.isEnabledFor(logging.DEBUG):
