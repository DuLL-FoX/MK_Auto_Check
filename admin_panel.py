...
class AdminPanel:
    ...
    def _cache_response(self, url: str, html: str) -> None:
        if len(self._response_cache) > 1000:
            num_to_evict = len(self._response_cache) // 5
            oldest_keys = sorted(self._response_cache.keys(),
                                 key=lambda k: self._response_cache[k][0])[:num_to_evict]
    ...
    def check_account_on_site(self, url: str, single_user: bool = False) -> Union[
        ...
        if single_user:
            ...
            result = self.aggregate_single_user_info(connections_data)
        ...
        if result["ban_counts"] > 0:
            if result["status"] != "banned":
                if result["ban_counts"] >= 5:
                    ...
                    result["status"] = "banned"
            elif result["status"] == "banned" and result[
                "ban_counts"] >= 5:
                result["status"] = "suspicious"
