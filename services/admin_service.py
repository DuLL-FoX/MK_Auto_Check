...
class StabilizedLoadOptimizer:
    ...
    def __init__(self, logger: logging.Logger, config: Any, initial_concurrency: int = 12) -> None:
        ...
        self.current_concurrency_level = max(self.min_concurrency, 4)
...
class AdminService:
    ...
    async def fetch_with_rate_limit(self, func: Callable, *args, **kwargs) -> Any:
        ...
        arg_parts_str = ":".join(map(str, args))[:200]
        kwarg_parts_str = ":".join(f"{k}={v}" for k, v in sorted(kwargs.items()))[:100]
    ...
    async def search_player(self, term: str, single_user: bool = True, max_depth: Optional[int] = None) -> Optional[
        ...
        try:
            async with asyncio.timeout(self._search_timeout):
                return await self._search_player_internal(term, single_user, max_depth)
        except asyncio.TimeoutError:
    ...
    async def _search_player_internal(self, term: str, single_user: bool = True, max_depth: Optional[int] = None) -> \
        ...
        if self.logger.isEnabledFor(logging.INFO):
            self.logger.info(
                ...
                f"(canonical: '{initial_term_canonical[:50]}'), single_user={single_user}, max_depth={effective_max_depth}"
            )

        raw_search_cache_key = f"search_player_v5:{initial_term_canonical}:single_user={single_user}:max_depth={effective_max_depth}"
        ...
        if search_cache_key in self._search_cache:
            ...
            if current_time - timestamp < self._search_cache_ttl:
                if self.logger.isEnabledFor(logging.DEBUG):
                    self.logger.debug(
                        f"Returning cached search result for '{initial_term_canonical[:50]}' from _search_cache."
                    )
                ...
                if current_depth < max_search_depth:
                    if isinstance(aggregated_data_for_term, dict):
                        ...
                        if all_connections_recent and aggregated_data_for_term["raw_html_snippet"]:
                            ...
                            if self._optimizer.current_delay > 0.5:
                                search_limit_for_depth = max(1, search_limit_for_depth - 1)
                                if self.logger.isEnabledFor(logging.DEBUG):
                                    self.logger.debug(
                                        f"High latency detected. Reducing search expansion limit to {search_limit_for_depth}.")

                            new_terms_to_queue.extend(extracted_identifiers_with_type[:search_limit_for_depth])
                            if new_terms_to_queue and self.logger.isEnabledFor(logging.DEBUG):
                                self.logger.debug(
                                    f"Identified {len(new_terms_to_queue)} new terms from '{current_term_str[:50]}' for depth {current_depth + 1}.")
                    elif self.logger.isEnabledFor(logging.DEBUG):
                        self.logger.debug(
                            f"Data for '{current_term_str[:50]}' (type: {type(aggregated_data_for_term)}) not dict, cannot extract new ids.")
                return {'result_data': aggregated_data_for_term, 'new_terms_to_search': new_terms_to_queue}
        except asyncio.TimeoutError:
            self.logger.error(
                f"Search term processing timed out for '{current_term_str[:50]}' at depth {current_depth}")
